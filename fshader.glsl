...
void main () 
{
  ...
  vec4 ambient1 = vec4(0.2, 0.2, 0.2, 1.0);
  ...
  if(Light==0)
  {	
  	COLOR = ambient1*color; // All
  }
  else if(Light==1)
  {	
  	COLOR = ((intensity1 * diffuse1  + spec1)*color + ambient1*color); // All
  }
  else if(Light==2)
  {
	COLOR = ((intensity2 * diffuse1  + spec2)*color + ambient1*color); // All
  }
  else
  {
	COLOR = ((intensity1 * diffuse1  + spec1)*color + (intensity2 * diffuse1  + spec2)*color + ambient1*color); // All
  }
  frag_color = COLOR;
}
